...
class Capture:
    ...
    def band_pulls(self, wrist_distance: float, left_elbow_angle: float = 0.0, right_elbow_angle: float = 0.0):
    ...
    def detect_punch(self, angle: float, angle_r: float, wrist: float, wrist_r: float, elbow: float, elbow_r: float):
        ...
        # Detect punch only when moving from 'rest' to 'punch'
        if (self.stage == 'rest' or self.stage is None) and (left_arm_extended or right_arm_extended):
            self.stage = 'punch'
            self.counter += 1
            print("Punch Detected! Rep Count:", self.counter)

        # Reset to 'rest' when both arms are no longer extended
